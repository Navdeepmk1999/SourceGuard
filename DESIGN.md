...
**Module 9 — Enterprise Security & Multi-Tenancy — ✅ Auth + RLS Implemented (Redis rate limiting still planned, see below)**
...
- **Known gaps, still not addressed:** `SUPABASE_URL` still needs to be added to `backend/.env` (the project's own URL, e.g. `https://<ref>.supabase.co` - not a secret, just needed to build the JWKS endpoint) - `get_current_user` 500s until it is. RLS itself exists only on `workspaces`, not `documents`/`document_chunks` - a defense-in-depth gap for any future endpoint that queries those tables without first calling `ensure_workspace_owner()`. `Workspace.name`'s uniqueness constraint is still global, not scoped per-user. The actual Postgres RLS policy and `set_config` wiring in `get_authenticated_db` remain untestable in this suite (SQLite has no RLS); only the SQLite-testable defense-in-depth layer (`ensure_workspace_owner()`) has committed coverage.

### Phase 2 Execution Roadmap (Planned)

**Module 9 (continued): Redis Rate Limiting**
- **Redis Rate Limiting:** Deploy a sliding-window algorithm via Upstash Redis to cap API requests, protecting upstream LLM quotas from Denial of Wallet (DoW) attacks. Not part of this pass - only the Supabase Auth + RLS portion of Module 9 (above) is implemented so far.

**Module 10: Contextual Intelligence & UX**
- **Persistent Conversation Memory:** Model `ChatSession` and `ChatMessage` relational schemas in PostgreSQL to maintain multi-turn chat history.
- **Sliding-Window Context:** Inject only the most recent conversational turns into the LLM prompt to optimize token consumption and maintain fast retrieval latency.
- **LangSmith Telemetry:** Instrument the RAG pipeline with observability tracing to evaluate token usage, retrieval precision, and generation latency in real-time.

**Module 11: Advanced Data Ingestion**
- **Layout-Aware Parsing:** Replace the baseline recursive text splitter with intelligent parsing (e.g., Unstructured.io) to handle complex document layouts.
- **Semantic Boundary Enforcement:** Chunk documents by natural elements like paragraphs, tables, and lists to drastically improve the quality and recall of pgvector embeddings.
